...
export default class Analysis {
  ...
  //
  // // Keeps track of whether we are in the main event loop.
  ...
  public literal: NPCallbacks.literal = (iid, val, hasGetterSetter) => {
    if (typeof val === "object") {
      ...
      for (const k in val) {
        ...
        // val might not have a prototype (or Object prototype)
  ...
  public getFieldPre: NPCallbacks.getFieldPre = (iid, receiver, offset, isComputed, isOpAssign, isMethodCall) => {
    // do injection here
    if (!receiver || this.spec.injectPropertyVal === undefined) return;

    let val = null;
    try {
      val = receiver[offset]; // this might not work in strict mode (e.g. accessing a functions arguments property)
    } catch (e) {
      return;
    }
  ...
  public invokeFunPre: NPCallbacks.invokeFunPre = (iid, f, receiver, args, isConstructor, isMethod) => {
    ...
    if (this.isNativeMap.get(f) || this.isNative(f)) {
      ...
      // Note from paul - I'm not completly sure what this is for -> but I don't believe it is needed for our purpose
      ...
      //         try {
      //           thisArg[field]; // try to access the field -> it might not work due to a getter exception (ignore for now)
      //         } catch (e) {
      //           continue; // ignore
      //         }
      ...
      //           if (typeof thisArg[field] === 'object' && thisArg[field] !== null) {
