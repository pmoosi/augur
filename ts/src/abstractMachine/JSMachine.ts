...
export default abstract class JSMachine<V, F> implements AbstractMachine {
  ...
  public functionInvokeStartOp: Operation<[string, number, number, DynamicDescription[], StaticDescription], void> =
    this.adviceWrap(
      ([name, expectedArgs, actualArgs, argShadowIDs, description]) => {
        ...
        let actualArgsValues =
          this.taintTree.get(this.ROOTID)
            .slice(this.taintTree.get(this.ROOTID).length - actualArgs)
            .map(v => this.sanitize(v, description));
        ...
        const functionInvocationTaint = this.join(
          this.taintTree.get(this.ROOTID)[this.taintTree.get(this.ROOTID).length - expectedArgs - 1],
        ...
        for (let i = this.taintTree.get(this.ROOTID).length - expectedArgs; i < this.taintTree.get(this.ROOTID).length; i++) {
          ...
          this.taintTree.get(this.ROOTID)[i] = this.join(
            this.taintTree.get(this.ROOTID)[i],
            functionInvocationTaint
          );
        }
  ...
  public functionInvokeEndOp: Operation<[string, DynamicDescription[], StaticDescription], void> =
    this.adviceWrap(
      ([name, shadowIDs, description]) => {
        ...
        if (this.returnValue) {
          returnTaintValue = this.sanitize(this.join(returnTaintValue, this.returnValue), description);
        }
  ...
  public promiseResolveOp: Operation<[number, any, DynamicDescription, StaticDescription], void> =
    this.adviceWrap(([id, promiseValue, promiseId, description]) => {
      this.promiseMap.set(promiseId, {
        resolve: this.varTaintMap.get(promiseValue),
  ...
  public promiseRejectOp: Operation<[number, any, DynamicDescription, StaticDescription], void> =
    this.adviceWrap(([id, promiseValue, promiseId, description]) => {
      this.promiseMap.set(promiseId, {
        resolve: this.getUntaintedValue(),
